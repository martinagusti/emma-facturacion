...
var AUTO_REFRESH_ON_FOCUS_BOUND=false;
var FOCUS_REFRESH_HINT_TIMEOUT=null;
var LAST_FOCUS_REFRESH_AT=0;
...
/* ═══════════════════════════════════════
   AUTH STATE
═══════════════════════════════════════ */
var auth={users:[],current:null,tab:'login',err:null,ok:null,sessionToken:''};
...
function getLastResponsable(){return S.lastResponsable||responsablesList()[0]||''}
function setLastResponsable(v){S.lastResponsable=v||'';try{if(v)storageSet('ov_last_responsable',v,true)}catch(e){}}
...
async function loadData(silent){
  ...
  try{
    var c=null,p=null,d=null,cm=null,lr=null;
    ...
    try{lr=await storageGet('ov_last_responsable',true)}catch(e){}
    ...
    S.lastResponsable=lr&&lr.value?lr.value:'';
...
/* ═══════════════════════════════════════
   AUTH ACTIONS
═══════════════════════════════════════ */
function applyAuthenticatedUser(user){if(!user){auth.current=null;return null}user.role=normalizeUserRole(user);user.isAdmin=user.role==='admin';if(user.role==='comercial'&&!user.commercialName)user.commercialName=normalizeCommercialName(user.name||userDisplayName(user.email));auth.current=user;return user}
async function doLogin(email,pass){
  auth.err=null;auth.ok=null;
  if(!email||!pass){auth.err='Rellena todos los campos';renderAuth();return}
  try{
    var data=await window.apiClient.login(email,pass);
    auth.sessionToken=data&&data.token?data.token:'';
    applyAuthenticatedUser(data&&data.user?data.user:null);
    await showApp();
  }catch(e){
    auth.err=e&&e.message?e.message:'No se ha podido iniciar sesi?n';renderAuth();
  }
}
async function doRegister(email,pass,pass2,adminKey){
  auth.err=null;auth.ok=null;
  if(!email||!pass){auth.err='Rellena email y contrase?a';renderAuth();return}
  if(pass!==pass2){auth.err='Las contrase?as no coinciden';renderAuth();return}
  if(pass.length<6){auth.err='M?nimo 6 caracteres';renderAuth();return}
  try{
    var data=await window.apiClient.register({email:email,password:pass,password2:pass2,adminKey:adminKey});
    auth.ok=data&&data.message?data.message:'Cuenta creada. Ya puedes entrar.';auth.tab='login';renderAuth();
  }catch(e){
    auth.err=e&&e.message?e.message:'No se ha podido registrar la cuenta';renderAuth();
  }
}
window.addEventListener('emma:unauthorized',function(){auth.err='La sesi?n ha expirado. Vuelve a iniciar sesi?n.';auth.ok=null;auth.current=null;auth.sessionToken='';auth.users=[];clearInterval(AUTO_INTERVAL);clearInterval(BACKUP_INTERVAL);var app=document.getElementById('appWrap');var authWrap=document.getElementById('authWrap');if(app) app.style.display='none';if(authWrap) authWrap.style.display='flex';renderAuth()});
...
async function doLogout(){
  auth.current=null;auth.sessionToken='';auth.users=[];
...
function shouldSkipAutoRefresh(){
  ...
  return !!(S.modal||document.getElementById('modalOverlay')||(active&&active.closest&&active.closest('.modal')));
}
function bindAutoRefreshOnFocus(){
  if(AUTO_REFRESH_ON_FOCUS_BOUND) return;
  AUTO_REFRESH_ON_FOCUS_BOUND=true;
  async function refreshOnFocus(){
    if(!auth.current||shouldSkipAutoRefresh()) return;
    if(Date.now()-LAST_FOCUS_REFRESH_AT<1200) return;
    LAST_FOCUS_REFRESH_AT=Date.now();
    await loadData(true);
    S.lastSyncHint='Sincronizado al volver';
    clearTimeout(FOCUS_REFRESH_HINT_TIMEOUT);
    FOCUS_REFRESH_HINT_TIMEOUT=setTimeout(function(){S.lastSyncHint='';renderApp()},6000);
    renderApp();
  }
...
function renderTopbar(){
  ...
  var syncHint=S.lastSyncHint?'<span> · '+esc(S.lastSyncHint)+'</span>':'';
  return '<div class="topbar">'+
    '<div class="brand">'+
      '<div><div class="brand-name">CONTROL FACTURACIÓN ONLINEVALLES</div></div>'+
    '</div>'+
    '<div class="topbar-right">'+
      '<div class="sync-info"><span class="sync-dot"></span> Actualizado '+lastStr+syncHint+'</div>'+
...
function bindEvents(){
  var r=root();
  var lb=document.getElementById('logoutBtn');if(lb) lb.addEventListener('click',function(){doLogout()});
