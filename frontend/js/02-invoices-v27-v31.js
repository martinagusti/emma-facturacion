...
async function saveInvoices(){try{await storageSet('ov_invoices',JSON.stringify(S.invoices||[]),true);S.lastSync=new Date();return true}catch(e){toast('Error al guardar facturas'+(e&&e.message?': '+e.message:''));return false}}
...
async function createInvoiceFromModal(){
  ...
  if(await saveInvoices()){closeModal();renderApp();}
}
async function setInvoiceStatus(id,status){
  ...
  if(await saveInvoices())renderApp();
}
...
bindEvents=function(){
  ...
  document.querySelectorAll('[data-f-period],[data-invoice-period]').forEach(function(el){el.addEventListener('click',function(){document.querySelectorAll('[data-f-period],[data-invoice-period]').forEach(function(b){b.classList.remove('on')});el.classList.add('on');var p=el.getAttribute('data-f-period')||el.getAttribute('data-invoice-period');document.getElementById('f_period').value=p;var wr=document.getElementById('f_split_wrap');if(wr)wr.classList.toggle('show',p==='fraccionado');if(p==='fraccionado')recalcSplitAmounts()})});
  ...
  document.querySelectorAll('[data-action="save-invoice"],[data-create-invoice]').forEach(function(el){el.addEventListener('click',createInvoiceFromModal)});
};
